...
def _get_polygon_pricing(
    ...
    *, logger: Logger = None
    ...
    logger and logger.info(f"Fetching from {unix_from} to {unix_to}")
    ...
    while 'next_url' in data and data['next_url']:
        logger and logger.info(f"Next page. Results count = {data['resultsCount'] if 'resultsCount' in data else None}")
        logger and logger.info(f"RESULTS[-5:] (total={len(data['results'])}) = \n{data['results'][-3:]}")
...
def get_polygon_pricing(
    ...
    *, logger: Logger = None
    ...
    return _get_polygon_pricing(ticker.upper(), unix_from, unix_to, timespan, True, logger=logger)
