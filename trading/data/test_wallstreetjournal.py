...
class TestWsj(unittest.TestCase):
    ...
    def test_merge_30m_to_1h(self):
        ...
        result = wallstreetjournal._merge_data_1h(input)
    ...
    def test_pricing_d1(self):
        now = time.time()
        lows, highs, vols, times = wallstreetjournal.get_pricing('nvda', now - 5*24*3600, now, interval=Interval.D1, return_quotes=['low', 'high', 'volume', 'timestamp'], skip_cache=config.test.skip_cache)
    ...
    def test_pricing_h1(self):
        now = time.time()
        lows, highs, vols, times = wallstreetjournal.get_pricing('nvda', now - 5*24*3600, now, interval=Interval.H1, return_quotes=['low', 'high', 'volume', 'timestamp'], skip_cache=config.test.skip_cache)
        self.assertTrue(lows and highs and vols and times)
    ...
    def test_pricing_m15(self):
        now = time.time()
        lows, highs, vols, times = wallstreetjournal.get_pricing('nvda', now - 3*24*3600, now, Interval.M15, return_quotes=['low', 'high', 'volume', 'timestamp'], skip_cache=config.test.skip_cache)
        self.assertTrue(lows and highs and vols and times)
    ...
    def test_pricing_m5(self):
        now = time.time()
        lows, highs, vols, times = wallstreetjournal.get_pricing('nvda', now - 3*24*3600, now, Interval.M5, return_quotes=['low', 'high', 'volume', 'timestamp'], skip_cache=config.test.skip_cache)
        self.assertTrue(lows and highs and vols and times)
