...
class BaseNewsProvider(NewsProvider):
    ...
    @override
    def get_news(self, security: Security, unix_from: float, unix_to: float, **kwargs) -> list[dict]:
        return self._get_news(security, unix_from, unix_to, **kwargs)
    @override
    def get_titles(self, security: Security, unix_from: float, unix_to: float, **kwargs) -> Sequence[str]:
        return [it['title'] for it in self.get_news(security, unix_from, unix_to, **kwargs)]
    ...
    def _get_news(self, security: Security, unix_from: float, unix_to: float, **kwargs) -> list[dict]:
        return self.get_news_raw(security, unix_from, unix_to, **kwargs)

    #region Abstract
    def get_news_persistor(self, security: Security) -> Persistor: raise NotImplementedError()
    def get_news_raw(self, security: Security, unix_from: float, unix_to: float, **kwargs) -> list[dict]: raise NotImplementedError()
    #endregion