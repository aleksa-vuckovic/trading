...
from trading.core.work_calendar import WorkCalendar, HolidaySchedule, BasicWorkCalendar
...
class NasdaqCalendar(WorkCalendar):
    ...
    def __init__(self):
        super().__init__(tz=work_calendar.ET)
    ...
    def get_next_timestamp(self, time: datetime|float|int, interval: Interval) -> datetime|float|int:
        ...
        if interval == Interval.L1:
            ...
            return self.month_end(timestamp+timedelta(days=15))
        if interval == Interval.W1:
            timestamp = self.week_end(time)
            if timestamp > time: return timestamp
            return self.week_end(timestamp+timedelta(days=7))
        if interval == Interval.D1:
            timestamp = time
...
class NasdaqCalendar(BasicWorkCalendar):
    ...
    def __init__(self):
        ...
        holidays.add_semi_days(
            '2021-11-26', '2022-11-25', '2023-07-03',
            '2023-11-24', '2024-07-03', '2024-11-29',
            '2024-12-24', '2025-07-03', '2025-11-28',
            '2025-12-24'
        )
        super().__init__(tz=dates.ET, open_hour=9, open_minute=30, close_hour=16, semi_close_hour=13, holidays=holidays)
NasdaqCalendar.instance = NasdaqCalendar()
