...
from trading.securities import FinancialTimes, Nasdaq

security = Nasdaq.instance.get_security('NVDA')
provider = FinancialTimes()
class TestFinancialtimes(unittest.TestCase):
    ...
    def test_pricing_m15(self):
        now = time.time()
        lows, highs, vols, times = provider.get_pricing(security, now - 3*24*3600, now, interval=Interval.M15, return_quotes=['low', 'high', 'volume', 'timestamp'], skip_cache=config.test.skip_cache)
        self.assertTrue(lows and highs and vols and times)
    ...
    def test_pricing_m5(self):
        now = time.time()
        lows, highs, vols, times = provider.get_pricing(security, now - 3*24*3600, now, interval=Interval.M5, return_quotes=['low', 'high', 'volume', 'timestamp'], skip_cache=config.test.skip_cache)
        self.assertTrue(lows and highs and vols and times)
